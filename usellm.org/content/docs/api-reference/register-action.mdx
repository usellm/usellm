...
In this example, the `replicateText` action takes a `text` input, makes a POST request to a Replicate model,
waits for the model to run, then retrieves and returns the model response. If the `text` input is not provided, an error is thrown.
