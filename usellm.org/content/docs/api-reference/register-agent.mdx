...
`llmService.registerAgent` is a method designed to register a new agent with a language learning model (LLM) service. This method allows you to specify a model, initial messages, and set of callable functions for a named agent.

### Import

```javascript
import { llmService } from "usellm";
```
...
Agents must be registered on the server-side. The following example shows how to register an agent using the `llmService.registerAgent` method:
